...
if(!function_exists('insert_ogp')) {
    ...
    function insert_ogp() {
        ...
        $og_image = wp_get_attachment_image_src($og_image,'full');
        $og_image = esc_url($og_image[0]);
...
<?php 
    ...
    $desc = get_field( 'description' );
    $corp_logo = wp_get_attachment_image_src($corp_logo,'full');
    $corp_logo = esc_url($corp_logo[0]);
